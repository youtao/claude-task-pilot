...
│   ├── design-to-tasks.md       # 设计文档转换代理 (NEW)
│   └── daily-reporter.md        # 日报生成代理
├── commands/
│   └── convert-design-to-tasks.md # 设计文档转换命令 (NEW)
├── hooks/
│   ├── session-start.md         # Session 启动钩子
│   ├── pre-tool-write.md        # 工具写入前钩子
│   └── post-tool-write.md       # 工具写入后钩子（含设计文档检测）
├── templates/
...
task_complexity_threshold: 4      # 小时数，超过则拆分
...
# 2. 设计文档创建到 docs/plans/2026-02-01-feature-design.md
...
```bash
# 检查配置
cat .claude/claude-task-pilot.local.md | grep auto_convert_designs
...
```bash
# 调整权重配置
# 编辑 .claude/claude-task-pilot.local.md
priority_weights:
  complexity: 0.4  # 增加复杂度权重
```
...
```markdown
# 手动编辑任务卡片调整依赖
# vim docs/todo/backlog/task-XXX-feature.md
```
